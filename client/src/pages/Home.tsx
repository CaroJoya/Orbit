  ...
  return (
    <Card className="metric-card">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 font-display text-[26px] font-semibold leading-none tracking-[-0.04em] text-ink">{value}</p>
...
function AppHeader({ mode, setMode, onRestart, destination }: { mode: Mode; setMode: (mode: Mode) => void; onRestart: () => void; destination: string }) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-[76px] items-center justify-between border-b border-ink/10 bg-paper/90 px-6 backdrop-blur-xl lg:px-9">
      <div className="flex items-center gap-3">
        <div className="brand-mark brand-mark-small"><span className="brand-glyph">N</span></div>
        <span className="font-display text-xl font-semibold tracking-[-0.04em] text-ink">Nomad<span className="font-sans font-bold text-teal">Sync</span></span>
      </div>
...
function Sidebar({ step, setStep }: { step: TravelerStep; setStep: (step: TravelerStep) => void }) {
  const [, setLocation] = useLocation();
  return (
    <aside className="fixed inset-y-0 left-0 z-50 hidden w-[252px] flex-col bg-ink px-5 py-6 text-paper lg:flex">
      <div className="flex items-center gap-3 px-2">
        <div className="brand-mark"><span className="brand-glyph">N</span></div>
        <div>
          <div className="font-display text-[22px] font-semibold tracking-[-0.05em]">Nomad<span className="font-sans text-teal-light">Sync</span></div>
  ...
  return (
    <div className="mb-8 flex items-center gap-0 overflow-x-auto pb-1 lg:mb-10">
      {journey.map((item, index) => {
        ...
        return (
          <div key={item.label} className="flex min-w-[96px] items-center">
...
function PriceBar({ total, delta }: { total: string; delta?: string }) {
  return (
    <div className="sticky bottom-4 z-30 flex items-center justify-between gap-4 rounded-[18px] bg-ink px-5 py-4 text-paper shadow-[0_14px_40px_rgba(23,34,35,0.22)] sm:px-6">
      ...
      <div className="flex items-end gap-3 text-right"><div><p className="text-2xl font-bold tracking-[-0.05em]">{total}</p>{delta && <p className="text-xs font-semibold text-teal-light">{delta}</p>}</div><ArrowDownRight size={17} className="mb-1 text-teal-light" /></div>
    </div>
  );
}
...
function ChatDrawer({ onClose, onAdd, added }: { onClose: () => void; onAdd: () => void; added: boolean }) {
  ...
  return <div className="drawer absolute inset-y-0 right-0 z-20 flex w-full max-w-[350px] flex-col bg-paper shadow-[-14px_0_36px_rgba(23,34,35,0.16)] sm:w-[350px] animate-slide-in"><div className="flex items-center justify-between border-b border-ink/10 px-5 py-5"><div><MiniLabel tone="saffron">Route waypoint · Chat</MiniLabel><h3 className="mt-1 font-display text-xl font-semibold tracking-[-0.04em]">Sharma Ji Samosa Hub</h3></div><button type="button" className="icon-button" onClick={onClose}><X size={16} /></button></div><div className="flex-1 overflow-y-auto"><img src={stopImage} alt="Samosa and tea at a roadside stop" className="h-32 w-full object-cover" /><div className="flex items-center justify-between px-5 pt-4"><span className="flex items-center gap-1.5 text-sm font-bold text-ink"><Star size={15} fill="currentColor" className="text-saffron" /> 4.8 hygiene score</span><span className="text-xs font-semibold text-moss">Open · 10:30–18:00</span></div><div className="space-y-3 p-5">{messages.map((message) => <div key={message.id} className={cn("flex", message.role === "user" ? "justify-end" : "justify-start")}>{message.role === "assistant" && <div className="mr-2 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-teal text-white"><Bot size={14} /></div>}<div className={cn("max-w-[220px] rounded-2xl px-3.5 py-2.5 text-sm leading-5", message.role === "user" ? "bg-teal text-white" : "bg-teal/8 text-ink")}>{message.text}</div></div>)}{typing && <div className="flex justify-start"><div className="mr-2 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-teal text-white"><Bot size={14} /></div><div className="flex items-center gap-1 rounded-2xl bg-teal/8 px-3.5 py-3"><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-teal [animation-delay:-0.2s]" /><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-teal [animation-delay:-0.1s]" /><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-teal" /></div></div>}</div></div><div className="border-t border-ink/10 p-5"><div className="field-shell mb-3"><input value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => event.key === "Enter" && sendMessage()} placeholder="Ask about hygiene, timing, price…" aria-label="Chat message" /><button type="button" onClick={sendMessage} className="icon-button h-8 w-8 shrink-0" aria-label="Send message"><Send size={14} className="text-teal" /></button></div><Button onClick={onAdd} className={cn("h-11 w-full rounded-xl font-bold", added ? "bg-moss text-white hover:bg-moss" : "bg-teal text-white hover:bg-teal-dark")}>{added ? <><Check size={16} className="mr-2" /> Added to route</> : <><Plus size={16} className="mr-2" /> Add to route · ₹240</>}</Button></div></div>;
}
  ...
  return (
    <div className="min-h-screen bg-paper text-ink">
      ...
      <div className="lg:pl-[252px]">
        ...
        <main className="main-shell mx-auto max-w-[1500px] px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
